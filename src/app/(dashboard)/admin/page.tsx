...
export default function AdminDashboard() {
  const [stats, setStats] = useState<AdminStats>({ students: 0, staff: 0, parents: 0, pendingAdmissions: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    apiGet<AdminStats>("/api/admin/stats")
      .then((data) => setStats(data))
      .catch(() => setStats({ students: 0, staff: 0, parents: 0, pendingAdmissions: 0 }))
