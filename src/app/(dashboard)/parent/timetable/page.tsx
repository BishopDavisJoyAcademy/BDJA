...
import {
  CalendarDays, Clock, MapPin, User, Printer, Loader2, GraduationCap, Users
} from "lucide-react";
...
export default function ParentTimetablePage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const [childrenRes, cfgRes] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const fetchTimetable = async () => {
      ...
      try {
        const headers = await getHeaders();
        const params = new URLSearchParams({ child_id: selectedChild, term: selectedTerm });
    ...
    fetchTimetable();
  }, [selectedChild, selectedTerm, getHeaders]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <CalendarDays className="w-6 h-6" style={{ color: GOLD }} /> Child&apos;s Timetable
          </h1>
          <p className="text-sm text-slate-400 mt-1">View your child&apos;s class schedule.</p>
        </div>
      ...
      {children.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-slate-400">No children linked</h3>
          <p className="text-sm text-slate-600 mt-2">You do not have any children linked to your account yet.</p>
        </div>
      ) : slots.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-slate-600 mt-2">The timetable for {selectedChildName} has not been published yet.</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 bg-white overflow-y-auto">
          <div className="max-w-5xl mx-auto p-8">
            ...
            <SchoolDocumentHeader
              ...
              subtitle={`${selectedChildName} — ${selectedTerm} ${config?.academic_year || ""}`}
            />
            <div className="overflow-x-auto">
