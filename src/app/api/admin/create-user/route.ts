...
export async function POST(req: NextRequest) {
  try {
    ...
    const newUser = await createUser({
      ...
      role: data.role,
      userCategory: role === "student" ? "student" : role === "parent" ? "parent" : "staff",
    ...
    await logAudit({
      ...
      userCategory: role === "student" ? "student" : role === "parent" ? "parent" : "staff", email: data.email, created_by_role: adminRole },
      ip_address: getClientIP(req),
      user_agent: req.headers.get("user-agent") || undefined,
    });
