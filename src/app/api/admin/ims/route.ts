...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await admin.from("inventory_items").select("*").order("created_at", { ascending: false });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error) return NextResponse.json({ error: error.message || "Failed to create item" }, { status: 500 });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { data: existing } = await admin.from("inventory_items").select("*").eq("id", id).single();
