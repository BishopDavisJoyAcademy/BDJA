...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.slug || !body.title || !body.content) return NextResponse.json({ error: "Slug, title, and content are required" }, { status: 400 });
    ...
    const { data, error } = await admin.from("cms_pages").insert([{
      ...
      meta_description: body.meta_description || null, is_published: body.is_published ?? false,
      created_by: session.userId,
    }]).select().single();
    if (error) return NextResponse.json({ error: "Failed to create page" }, { status: 500 });
    await logAudit({ user_id: session.userId, action: "CMS_PAGE_CREATED", table_name: "cms_pages", record_id: data.id, new_data: { slug: body.slug, title: body.title }, ip_address: getClientIP(req) });
...
export async function PUT(req: NextRequest) {
  try {
    ...
    await logAudit({ user_id: session.userId, action: "CMS_PAGE_UPDATED", table_name: "cms_pages", record_id: id, ip_address: getClientIP(req) });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await logAudit({ user_id: session.userId, action: "CMS_PAGE_DELETED", table_name: "cms_pages", record_id: id, ip_address: getClientIP(req) });
