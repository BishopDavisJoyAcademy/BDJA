          ...
          <div className="flex flex-wrap items-center gap-4">
            {topLinks.map((link) => {
              ...
              if (link.action === "contact") {
                return (
                  <button key={link.label} onClick={() => setContactOpen(true)} className="flex items-center gap-1.5 text-slate-400 hover:text-amber-400 transition-colors text-xs">
              ...
              return (
                <Link key={link.label} href={link.href!} className="flex items-center gap-1.5 text-slate-400 hover:text-amber-400 transition-colors text-xs">
                  <Icon className="w-3.5 h-3.5" />
                  <span className="hidden sm:inline">{link.label}</span>
                </Link>
              );
              ...
              <div className="max-w-7xl mx-auto px-4 py-3">
                <div className="relative max-w-md">
                  ...
                  <input
                    ...
                    autoFocus
                    className="w-full pl-10 pr-4 py-2 bg-slate-800/50 border border-slate-700/50 rounded-xl text-sm text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500/30"
      ...
      <header className="relative z-40 shrink-0 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/30">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-1">
            ...
            <Link
              href="/admissions"
              className="ml-2 px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 font-medium rounded-lg text-sm transition-colors"
            >
              Apply Now
            </Link>
          </nav>
            ...
            >
              <nav className="max-w-7xl mx-auto px-4 py-3 space-y-1">
                ...
                <Link
                  href="/admissions"
                  onClick={() => setMobileMenuOpen(false)}
                  className="block px-3 py-2.5 bg-amber-500/10 text-amber-400 font-medium rounded-lg text-sm mt-2"
                >
                  Apply Now
                </Link>
              </nav>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
            <div className="lg:col-span-1">
              ...
              <div className="space-y-2">
                <input type="email" placeholder="Enter your email" className="w-full px-3 py-2 rounded-xl text-sm text-white placeholder-slate-600 bg-slate-800/40 border border-slate-700/40 focus:outline-none focus:border-amber-500/30 transition-colors" />
                <button className="w-full px-3 py-2 text-sm font-medium text-slate-950 bg-amber-500 hover:bg-amber-400 rounded-xl transition-colors">Subscribe</button>
              </div>
            </div>
            <div>
              ...
              <ul className="space-y-2">
                {["About Us", "Academics", "Admissions", "Students", "News & Events", "Contact Us"].map((item) => (
                  <li key={item}>
                    <Link href={`/${item.toLowerCase().replace(/ & /g, "-").replace(/ /g, "-")}`} className="text-slate-500 text-xs hover:text-amber-400 transition-colors">
                      {item}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="space-y-2">
                {["VORA", "Student Email", "Student Portal", "Library", "Help Desk", "Downloads"].map((item) => (
                  <li key={item}>
                    <Link href={item === "Student Portal" ? "/login" : `/${item.toLowerCase().replace(/ /g, "-")}`} className="text-slate-500 text-xs hover:text-amber-400 transition-colors">
                      {item}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="space-y-2">
                {["Downloads", "Policies", "Calendar", "Photo Gallery", "FAQs"].map((item) => (
                  <li key={item}>
                    <Link href={`/${item.toLowerCase().replace(/ /g, "-")}`} className="text-slate-500 text-xs hover:text-amber-400 transition-colors">
                      {item}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="space-y-3">
                ...
                <li><a href="tel:0708449158" className="text-slate-500 text-xs hover:text-amber-400 transition-colors">0708 449 158</a></li>
                <li><a href="mailto:bishopdavisjoyacademy@gmail.com" className="text-slate-500 text-xs hover:text-amber-400 transition-colors">bishopdavisjoyacademy@gmail.com</a></li>
              ...
              {contactSent ? (
                ...
                <>
                  <h3 className="text-base font-semibold text-white mb-1 flex items-center gap-2">
                    <Send className="w-4 h-4 text-amber-400" /> Send a Message
                  </h3>
                  <p className="text-slate-500 text-xs mb-5">Reach out to Bishop Davis Joy Academy directly.</p>
                  <form onSubmit={handleContactSubmit} className="space-y-3">
                    <input type="text" placeholder="Your Name" required value={contactForm.name} onChange={e => setContactForm({...contactForm, name: e.target.value})} className="w-full px-4 py-2.5 rounded-xl text-sm text-white placeholder-slate-600 bg-slate-800/40 border border-slate-700/40 focus:outline-none focus:border-amber-500/30 transition-colors" />
                    <input type="email" placeholder="Your Email" required value={contactForm.email} onChange={e => setContactForm({...contactForm, email: e.target.value})} className="w-full px-4 py-2.5 rounded-xl text-sm text-white placeholder-slate-600 bg-slate-800/40 border border-slate-700/40 focus:outline-none focus:border-amber-500/30 transition-colors" />
                    <textarea placeholder="Your Message" required rows={4} value={contactForm.message} onChange={e => setContactForm({...contactForm, message: e.target.value})} className="w-full px-4 py-2.5 rounded-xl text-sm text-white placeholder-slate-600 bg-slate-800/40 border border-slate-700/40 focus:outline-none focus:border-amber-500/30 transition-colors resize-none" />
                    <button type="submit" className="w-full flex items-center justify-center gap-2 px-6 py-2.5 text-slate-950 font-semibold bg-amber-500 hover:bg-amber-400 rounded-xl transition-colors text-sm">
