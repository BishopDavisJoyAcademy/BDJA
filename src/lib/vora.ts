...
// JSON file content extends VoraContent with extra fields
interface VoraJsonItem extends VoraContent {
  category?: string;
  tags?: string[];
...
function loadGradeFile(grade: string): VoraJsonItem[] {
  const filePath = path.join(CONTENT_DIR, GRADE_FILES[grade] || `${grade}.json`);
  if (!fs.existsSync(filePath)) return [];
  try {
    const raw = fs.readFileSync(filePath, "utf-8");
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? parsed.map((item: unknown) => ({ ...(item as Record<string, unknown>), grade_level: grade } as VoraJsonItem)) : [];
  } catch {
    return [];
  }
}

export function loadAllVoraContent(): VoraJsonItem[] {
  const all: VoraJsonItem[] = [];
...
export function getVoraByGrade(grade: string): VoraJsonItem[] {
