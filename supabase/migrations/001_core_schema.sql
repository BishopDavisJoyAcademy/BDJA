-- ============================================================
-- 001_core_schema
-- Complete database schema: 60+ tables, RLS policies, functions, triggers, auth hooks, security layer, attachments bucket.
--
-- BDJA Platform — Version 1.0.0
-- Consolidated from: 001_initial_schema, 004_complete_reset, 008 (folded), 014 (folded), 20240907_add_campus_is_active (folded), phase7 subjects/classes/students/library column adds (folded)
--
-- ORDER MATTERS: run 001 → 009 in sequence on a fresh database.
-- ============================================================

-- ============================================================
-- BDJA Core Schema — full platform structure.
-- Safe to run on a fresh project. Replaces the legacy reset-based migrations.
-- ============================================================

-- ============================================
-- PHASE 1: DROP EVERYTHING OLD
-- ============================================
...
BEGIN
  FOR func IN
    ...
    AND proname NOT IN ('gen_random_uuid')
  LOOP
...
CREATE TABLE IF NOT EXISTS subjects (
  ...
  name TEXT NOT NULL
  color TEXT,
  curriculum_strands JSONB,
  description TEXT,
  grading_scales JSONB,
);
...
CREATE TABLE IF NOT EXISTS joy_user_preferences (
  ...
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  user_id UUID NOT NULL,
  CONSTRAINT joy_user_preferences_user_id_key UNIQUE (user_id),
  CONSTRAINT joy_user_preferences_user_id_fkey FOREIGN KEY (user_id) REFERENCES profiles(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_joy_user_preferences_user_id ON joy_user_preferences(user_id);
